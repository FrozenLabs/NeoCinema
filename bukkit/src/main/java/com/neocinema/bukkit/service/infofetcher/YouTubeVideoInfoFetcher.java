...
public class YouTubeVideoInfoFetcher extends VideoInfoFetcher {
    ...
    public YouTubeVideoInfoFetcher(NeoCinemaPlugin neoCinemaPlugin, String youtubeVideoId) {
        super("neocinema.request.youtube");
        this.neoCinemaPlugin = neoCinemaPlugin;
        this.youtubeDataApiKey = neoCinemaPlugin.getneocinemaConfig().youtubeDataApiKey;
    ...
    public CompletableFuture<VideoInfo> fetch() {
        ...
        if (!keyConfigured()) {
            neoCinemaPlugin.getLogger().warning("A YouTube video was unable to be requested. You must set a YouTube Data API V3 key in your neocinema config.yml. Falling back to Piped and Invidious backends");
