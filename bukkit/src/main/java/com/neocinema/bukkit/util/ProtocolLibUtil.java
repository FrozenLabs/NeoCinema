...
public final class ProtocolLibUtil {
    ...
    public static void registerSoundPacketListener(NeoCinemaPlugin neoCinemaPlugin) {
        getProtocolManager().addPacketListener(
                new PacketAdapter(neoCinemaPlugin, ListenerPriority.NORMAL, PacketType.Play.Server.NAMED_SOUND_EFFECT) {
                    @Override
                    public void onPacketSending(PacketEvent event) {
                        if (event.getPacketType() == PacketType.Play.Server.NAMED_SOUND_EFFECT) {
                            ...
                            Theater playerTheater = neoCinemaPlugin.getTheaterManager().getCurrentTheater(player);
